...
from models.VCD.vcd_sample import evolve_vcd_sampling
from models.VCD.vcd_add_noise import add_diffusion_noise
from models.VCD.llava.constants import IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
from models.VCD.llava.conversation import conv_templates, SeparatorStyle
from models.VCD.llava.model.builder import load_pretrained_model
from models.VCD.llava.utils import disable_torch_init
from models.VCD.llava.mm_utils import tokenizer_image_token, get_model_name_from_path, KeywordsStoppingCriteria
evolve_vcd_sampling() #enable vcd
...
def main(args):
    # load model
    disable_torch_init()
    model_path = "/data2/fyx/llava-v1.5-7b"
    ...
    for idx, img_id in tqdm(enumerate(range(len(img_files))), total=len(img_files)):
        ...
        conv.append_message(conv.roles[0], qs + "Describe the image.")
